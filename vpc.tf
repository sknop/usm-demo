...
resource "aws_subnet" "usm-public-subnet" {
  vpc_id = aws_vpc.vpc.id
...
resource "aws_route_table_association" "public-subnet-route-table-association" {
  count = length(aws_subnet.usm-public-subnet)
  subnet_id = aws_subnet.usm-public-subnet.*.id[count.index]
  route_table_id = aws_route_table.public-route-table.id
}


resource "aws_subnet" "usm-private-subnet" {
  vpc_id = aws_vpc.vpc.id
  count = length(var.private-subnets-cidr)
  cidr_block = var.private-subnets-cidr[count.index]
  availability_zone = var.private-availability-zones[count.index]
  map_public_ip_on_launch = false

  tags = {
    Name = "USM Private Subnet ${count.index}"
  }
}
...
resource "aws_security_group" "all-usm" {
  name = "all-usm-sg"
  description = "Allows free traffic between all instances within the USM VPC"
  vpc_id = aws_vpc.vpc.id
